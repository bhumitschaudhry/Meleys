...
- **[Installation & Setup](./setup.md)** — Prerequisites, compilation, and quick start.
- **[Configuration](./configuration.md)** — `config.toml` options and environment variables.
- **[HTTP REST API Reference](./api.md)** — Endpoints, request schemas, and error codes.
- **[MCP Reference](./mcp.md)** — Tool definitions and stdio protocol setup for coding agents.
- **[Architecture & Design](./architecture.md)** — Process isolation, thread safety, and engine abstraction.
...
```
*(Or run `meleys setup install` manually. See [Setup](./setup.md).)*
...
[Apache 2.0](https://github.com/bhumitschaudhry/Meleys/blob/main/LICENSE)
